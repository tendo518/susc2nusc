...
# refer to https://github.com/rerun-io/rerun/tree/latest/examples/python/nuscenes_dataset
...
def ensure_scene_available(
    ...
    try:
        nusc = nuscenes.NuScenes(
            version=dataset_version, dataroot=root_dir, verbose=True
        )
    except AssertionError as e:  # dataset initialization failed
        print(f"Could not find dataset at {root_dir}: {e}")
        sys.exit(1)

    scene_names = [s["name"] for s in nusc.scene]
    if scene_name not in scene_names:
        raise ValueError(
            f"{scene_name=} not found in dataset. Available: {scene_names}"
        )
...
def log_nuscenes(nusc: nuscenes.NuScenes, scene_name: str, max_time_sec: float) -> None:
    ...
    # log_radars(first_radar_tokens, nusc, max_timestamp_us) # No Radar
...
def log_lidar_and_ego_pose(
    ...
    while current_lidar_token != "":
        ...
        lat = ego_pose.get("lat")
        lng = ego_pose.get("lng")
...
def log_cameras(
    first_camera_tokens: list[str], nusc: nuscenes.NuScenes, max_timestamp_us: float
) -> None:
    """Log camera data."""
    for first_camera_token in first_camera_tokens:
        ...
        while current_camera_token != "":
            ...
            data_file_path = nusc.dataroot / sample_data["filename"]
            rr.log(
                f"world/ego_vehicle/{sensor_name}", rr.EncodedImage(path=data_file_path)
            )
            current_camera_token = sample_data["next"]
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Visualizes the nuScenes dataset using the Rerun SDK."
    )
    parser.add_argument(
        "--root-dir",
        type=pathlib.Path,
    ...
    nusc = nuscenes.NuScenes(
        version=args.dataset_version, dataroot=args.root_dir, verbose=True
    )
    ...
    sensor_views = [
        rrb.Spatial2DView(
            ...
            overrides={
                "world/anns": rr.Boxes3D.from_fields(fill_mode="majorwireframe")
            },
        )
